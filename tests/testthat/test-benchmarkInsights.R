...
test_that("addMetadata handles multiple entries correctly", {
  ...
  expect_equal(
    benchmark$metadata$speciesTissue,
    c("Human breast cancer", "Mouse brain")
  )
  expect_equal(benchmark$metadata$protocol, c("Visium", "ST"))
  expect_equal(
    benchmark$metadata$DOI,
    c("10.1038/s41588-021-00911-1", "10.1016/j.cell.2021.05.015")
  )
})
...
test_that("getScatterplot creates a scatterplot with optional grouping", {
  evalResult1 <- data.frame(
    ...
    evidence = c(200000, 400000, 600000, 200000, 400000, 600000),
    metric = rep("time", 6),
  ...
  line_plot <- benchmark$getLineplot(
    benchmark$evalSummary,
    metricVariable = "time"
  )
