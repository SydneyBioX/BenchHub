predict_mpop <- function(mpop_result, newx, s = "lambda.min") {
  ...
  if (mpop_result$mpop_mode == "glmnet") {
    result1_link <- predict(
      object = mpop_result$model1,
      newx = newx[, mpop_result$feature, drop = FALSE],
      s = s, type = "link"
    )
    result2_link <- predict(
      object = mpop_result$model2,
      newx = newx[, mpop_result$feature, drop = FALSE],
      s = s, type = "link"
    )
  } else {
    result1_link <- stats::predict(
      object = mpop_result$model1,
      newx = newx[, mpop_result$feature, drop = FALSE],
      type = "link"
    )
    result2_link <- stats::predict(
      object = mpop_result$model2,
      newx = newx[, mpop_result$feature, drop = FALSE],
      type = "link"
    )
  ...
  if (mpop_result$family_params$family == "binomial") {
    if (mpop_result$mpop_mode == "glmnet") {
      result1_prob <- predict(
        object = mpop_result$model1,
        newx = newx[, mpop_result$feature, drop = FALSE],
        s = s, type = "response"
      )
      result2_prob <- predict(
        object = mpop_result$model2,
        newx = newx[, mpop_result$feature, drop = FALSE],
        s = s, type = "response"
      )
    ...
    mpop_model_avg_class <- dplyr::case_when(
      mpop_model_avg_prob <= 0.5 ~ mpop_result$family_params$factor_levels[1],
    ...
    tib_result <- tib_result %>%
      dplyr::mutate(
        mpop_model_avg_prob = mpop_model_avg_prob,
