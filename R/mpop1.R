mpop1 = function(x1, x2, y1, y2, family, alpha = 1,
  ...
  for (i in 1:n_iter) {
    ...
    message("MPOP1 - Step ", sprintf("%02d", i),
    ...
    selected_features = c(selected_features,
                          base::intersect(rownames(zs1_coef), rownames(zs2_coef))) %>% unique
  ...
  if (length(selected_features) == 0) {
    ...
    step_features_tbl = step_features_tbl %>%
      dplyr::mutate(ever_selected_features = .data$feature_name %in% selected_features,
...
feature_tibble = function(zs_coef, coef_model = NA) {
  if (nrow(zs_coef) == 0) {
    zs_coef_tbl = tibble::tibble(coef_model = coef_model,
                                 feature_name = NA,
                                 feature_value = NA)
  } else {
    zs_coef_tbl = tibble::tibble(coef_model = coef_model,
...
mpop1_iterate = function(x1, x2, y1, y2, family, alpha = 1,
  ...
  for (i in alpha) {
    ...
    mpop1_result = mpop1(x1 = x1, x2 = x2, y1 = y1, y2 = y2,
    ...
    if (length(all_selected_features) == 0) {
      ...
      return(tibble::lst(mpop1_features = all_selected_features,
