...
evalTask_obj_generator <- function(task_name, gs_result) {
  ...
  eval_task_obj <- eval_task_factory$new(
    task_name = task_name,
    gs_result = gs_result,
...
evalTrio_obj_generator <- function(eval_data, data_name, organism,
  ...
  eval_trio_obj <- eval_trio_factory$new(
    eval_data = eval_data,
    eval_task_list = list,
    meta_info = meta_info
  )
