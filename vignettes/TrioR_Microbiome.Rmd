...
```{r}
# initiate a Trio object
trio <- Trio$new(data = lubomPD, datasetID = "lubomPD")
```
...
trio$addAuxData(name = "lubomski_microbiome", auxData = lubomPD, metrics = "Balanced Accuracy")
```
...
for (i in seq_along(cv_ind)) {
  ...
  eval_res <- trio$evaluate(list("lasso" = list(lubomski_microbiome = pred)), splitIndex = i)
...
After cross-validation, users can visualise cross-validation results by averaging results across folds within each repeats.

```{r}

result$fold <- unlist( lapply ( strsplit( result$track, ".", fixed = T ), `[`, 1))

result$repeats <- unlist( lapply ( strsplit( result$track, ".", fixed = T ), `[`, 2))

result <- result %>% 
   dplyr::group_by(datasetID, method, auxData, metric, repeats) %>%
   dplyr::summarize(mean_result = mean(result ))

boxplot(result$mean_result )
```
