...
```{r}
<<<<<<< Updated upstream:vignettes/WIP/_v3_intro_bmi.Rmd
...
>>>>>>> Stashed changes:vignettes/v3_intro_bmi.Rmd
```
...
```{r}
bmi$getLineplot(bmi$evalSummary[bmi$evalSummary$metric == "memory" , ])
```
...
```{r}
bmi$getBoxplot(bmi$evalSummary[bmi$evalSummary$metric == "KDEstat" & bmi$evalSummary$auxData == "scaledVar", ])
```
