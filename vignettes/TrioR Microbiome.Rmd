...
`TrioR` can take datasets provided by users. To demonstrate, its ability to take user-provided datasets, we'll be using a microbiome dataset called `Lubomski` obtained from the `PD16Sdata` package. The following code will import the `Lubomksi` data into R.
...
The task we'll be evaluating using `TrioR` is a binary classification task where each sample is either a Parkinson's Disease (PD) patient or Healthy Control (HC). The following code will extract PD status as a vector for classification from the `Lubomski` data and store it as a vector named `lubom_pd` where `1` represents `PD` and `0` represents `HC`.
...
Once the data are ready to be inputted to `TrioR`, we can load `TrioR`.
