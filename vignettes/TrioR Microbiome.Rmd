...
Once we create a TrioR object, we can add a metric to it using the `addMetric()` function. Users can use existing evaluation functions by specifying `metric`, and can name their evaluation metric by specifying `name`. Since the evaluation task is a classification task, we chose accruacy as our evaluation metric. In this case, we created a user-defined accruacy calculation function, and passed it to the `TrioR` object, naming it `Accuracy`.

```{r}
# create a user-defined evaluation metric function
compute_acc = function(ref, pred) {
  acc = mean(ref == pred)
  return(acc)
}

# add a metric to the Trio object
trio$addMetric(name = "Accuracy", metric = compute_acc)
```
...
trio$addAuxData(name = "lubomski_microbiome", auxData = lubomPD, metrics = "Accuracy")
```
...
for (i in 1:length(cv_ind)) {
  ...
  trio$addAuxData(name = "lubomski_microbiome", auxData = y_test, metrics = "Accuracy")
  
  # get the chosen evaluation metric from the Trio
  cv_res[i] <- trio$evaluate(list(lubomski_microbiome = pred))
}
```

After cross-validation, users can visualise cross-validation results. Note that the output of the `evaluate()` function is a list, so we unlist `cv_res` to visualise it.

```{r}
boxplot(unlist(cv_res))
```
