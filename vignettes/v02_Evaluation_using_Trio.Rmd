...
The supporting evidence is the patient status, which is the `lubomPD` vector that we extracted from the `Lubomski` data above. Similar to adding a metric, users can name their supporting evidence by specifying `name`. Note that users need to specify the evaluation metric to be used through the `metrics` argument, which has to match the evaluation metric added to the `Trio` object.  

```{r}
# add supporting evidence to the Trio object
# name is the user-defined name, can be anything
# evidence is the value of the supporting evidence
# metric needs to be same name of the metric that has been added to the Trio object
trio$addEvidence(
  name = "patient_status", evidence = lubomPD, metrics = "Balanced Accuracy"
)
```
