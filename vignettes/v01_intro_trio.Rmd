...
- `Trio` object constructs benchmarking data by organising data, evaluation metrics, and supporting evidence (gold standards).    
...
The Trio class is designed to facilitate the storing and sharing of benchmarking datasets. 
Each Trio is structured around a single dataset but can include multiple metrics and multiple pieces of supporting evidence (such as references or gold standards). 
...
You can directly use the name from the Curated Trio Datasets sheet to initialise a Trio object populated with some metrics and supporting evidence. This method is useful when you want to quickly start with a predefined dataset.
...
The above output shows that we have a Trio with a dataset, metrics, and supporting evidence. The dataset contains 137 rows and 9 columns, and the metrics and supporting evidence are already populated and printed.
...
## Adding and getting `evidence`

`evidence` (supporting evidence) refers to gold standards in evaluation tasks and falls into two categories:
...
First we add some supporting evidence to the Trio object to evaluate against.

Supporting evidence is any data that allows you to make an evaluation. Supporting evidence is linked to metrics that allow you to evaluate it.

```{r}
# It can be a fixed quantity
trio$addEvidence(
  name = "Number One", evidence = 1, metrics = c("equality", "inequality")
)

# Or it can be a function that acts on the data
trio$addEvidence(
  name = "length", evidence = length, metrics = c("equality", "inequality")
)
```

To view the supporting evidence one can use `getEvidence`.

```{r}
trio$getEvidence("length") # This is equivalent to `length(trio$data)`
```

## Evaluation Example

Once Trio is set up with relevant metrics and supporting evidence, we are ready to evaluate.
